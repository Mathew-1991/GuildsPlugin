...
public class GuildsPlugin extends JavaPlugin {
    ...
    /**
     * Helper: get configured tag color for a guild (default YELLOW).
     */
    public ChatColor getGuildTagColor(Guild guild) {
        if (guild == null) return ChatColor.YELLOW;

        String path = "guilds-extra." + guild.getName() + ".tagcolor";
        String raw = getConfig().getString(path, "YELLOW");
        if (raw == null) return ChatColor.YELLOW;

        try {
            ChatColor c = ChatColor.valueOf(raw.toUpperCase());
            return c.isColor() ? c : ChatColor.YELLOW;
