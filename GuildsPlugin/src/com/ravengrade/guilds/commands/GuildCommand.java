...
public class GuildCommand implements CommandExecutor, TabCompleter {
    ...
    private void sendHelp(Player p) {
        ...
        p.sendMessage(ChatColor.YELLOW + "/guild seticon <material>");
        p.sendMessage(ChatColor.YELLOW + "/guild tagcolor <color>");
    ...
    public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
        ...
        switch (sub) {
            ...
            case "seticon": {
                ...
                if (g.getRank(uuid) != GuildRank.LEADER) {
                    p.sendMessage(ChatColor.RED + "Only the leader can set the guild icon.");
                    return true;
                }
                if (args.length < 2) {
                    p.sendMessage(ChatColor.RED + "Usage: /guild seticon <material>");
                    return true;
                }
                ...
                String path = "guilds-extra." + g.getName() + ".icon";
                plugin.getConfig().set(path, mat.name());
                plugin.saveConfig();

                p.sendMessage(ChatColor.GREEN + "Guild icon set to " + mat.name() + ".");
                return true;
            }
            ...
            case "tagcolor": {
                ...
                if (g.getRank(uuid) != GuildRank.LEADER) {
                    p.sendMessage(ChatColor.RED + "Only the leader can change the guild tag color.");
                    return true;
                }
                if (args.length < 2) {
                    p.sendMessage(ChatColor.RED + "Usage: /guild tagcolor <color>");
                    p.sendMessage(ChatColor.YELLOW + "Examples: red, gold, yellow, green, blue, aqua, dark_red, dark_green, gray, white, light_purple");
                    return true;
                }
                ...
                try {
                    ...
                    p.sendMessage(ChatColor.RED + "Unknown color: " + args[1]);
                    p.sendMessage(ChatColor.YELLOW + "Examples: red, gold, yellow, green, blue, aqua, dark_red, dark_green, gray, white, light_purple");
                    return true;
                }

                if (!color.isColor()) {
                    p.sendMessage(ChatColor.RED + "That is not a valid chat color.");
                    return true;
                }

                String path = "guilds-extra." + g.getName() + ".tagcolor";
                plugin.getConfig().set(path, color.name());
                plugin.saveConfig();

                p.sendMessage(ChatColor.GREEN + "Guild tag color set to " + color + color.name() + ChatColor.RESET + ".");
                updateAllTabs();
                return true;
            }
    ...
    public List<String> onTabComplete(CommandSender sender, Command command, String alias, String[] args) {
        ...
        if (args.length == 1) {
            List<String> base = Arrays.asList(
                    ...
                    "motd", "invitemode", "seticon", "tagcolor", "rename",
                    "disband",
                    "gchat", "menu", "list"
            );
        ...
        if (args.length == 2) {
            ...
            if (sub.equals("tagcolor")) {
                List<String> colors = Arrays.asList(
                        "red", "gold", "yellow", "green", "blue",
                        "aqua", "dark_red", "dark_green", "gray",
                        "white", "light_purple"
                );
                String start = args[1].toLowerCase(Locale.ROOT);
                for (String c : colors) {
                    if (c.startsWith(start)) out.add(c);
                }
            }
        }
